...
class RemoveMultiRoadLanes(AtomicBehavior):
    """
    Updates the blackboard to tell the background activity to remove its actors from the given 
    multiple lanes and stop generating new ones on this lane, or recover from stopping.

    Args:
        lane_wp (List[carla.Waypoint]): List of carla.Waypoints
        active (bool)
    """
    def __init__(self, lane_wps, name="BA_RemoveMultiRoadLanes"):
        self._lane_wps = lane_wps
        super().__init__(name)

    def update(self):
        """Updates the blackboard and succeds"""
        py_trees.blackboard.Blackboard().set("BA_RemoveMultiRoadLanes", self._lane_wps, overwrite=True)
        return py_trees.common.Status.SUCCESS

class ReAddMultiRoadLanes(AtomicBehavior):
    """
    Updates the blackboard to tell the background activity to readd the ego road lane.
    ...
    Args:
        ...
        active (bool)
    """
    def __init__(self, offsets, name="BA_ReAddMultiRoadLanes"):
        self._offsets = offsets
        super().__init__(name)

    def update(self):
        """Updates the blackboard and succeds"""
        py_trees.blackboard.Blackboard().set("BA_ReAddMultiRoadLanes", self._offsets, overwrite=True)
        return py_trees.common.Status.SUCCESS
