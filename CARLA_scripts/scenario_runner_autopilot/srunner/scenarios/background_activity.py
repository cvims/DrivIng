...
class BackgroundBehavior(AtomicBehavior):
    ...
    def __init__(self, ego_actor, route, debug=False, name="BackgroundBehavior"):
        ...
        # self._attribute_filter = {'has_lights': True, }
    ...
    def _update_parameters(self):
        ...
        # Remove multiple road lanes
        remove_multi_lane_data = py_trees.blackboard.Blackboard().get('BA_RemoveMultiRoadLanes')
        if remove_multi_lane_data is not None:
            for lane_wp in remove_multi_lane_data:
                self._remove_road_lane(lane_wp)
            py_trees.blackboard.Blackboard().set('BA_RemoveMultiRoadLanes', None, True)
        ...
        # Readd multiple road lanes
        readd_multi_lane_data = py_trees.blackboard.Blackboard().get('BA_ReAddMultiRoadLanes')
        if readd_multi_lane_data is not None:
            for lane_offset in readd_multi_lane_data:
                self._readd_road_lane(lane_offset)
            py_trees.blackboard.Blackboard().set('BA_ReAddMultiRoadLanes', None, True)
    ...
    def _spawn_actor(self, spawn_wp, ego_dist=0):
        ...
        #     attribute_filter={'has_lights': True}, tick=False
        # )             
                
        actor = CarlaDataProvider.request_new_actor(
            'vehicle.*', spawn_transform, 'background', True,
            attribute_filter={}, tick=False
        )
