...
class StartAIWalkerControll(AtomicBehavior):
    def __init__(self, controllers, walker_data, name="StartAIWalkerControll"):
        super(StartAIWalkerControll, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self.walker_controllers = controllers
        self.walker_data = walker_data
        self._world = CarlaDataProvider.get_world()
        
        
    def update(self):
        new_status = py_trees.common.Status.RUNNING
        for i, controller in enumerate(self._world.get_actors(self.walker_controllers)):
            controller.start()
            controller.go_to_location(self.walker_data[i]["destination"])
            controller.set_max_speed(self.walker_data[i]["speed"])
        new_status = py_trees.common.Status.SUCCESS
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
        return new_status
    
class BatchStopAIWalkerControll(AtomicBehavior):
    def __init__(self, walker_controllers, name="BatchStopAIWalkerControll"):
        super(BatchStopAIWalkerControll, self).__init__(name)
        self.walker_controllers = walker_controllers
        self._world = CarlaDataProvider.get_world()
...
class BatchActorDestroy(AtomicBehavior):
    """
    This class contains destroying multiple carla actor behavior.

    Important parameters:
    - list(actor): list of CARLA actor to be deleted

    The behavior terminates after removing the actor
    """
    def __init__(self, actors, name="BatchActorDestroy"):
        """
        Setup actor
        """
        self._actors = actors
        super(BatchActorDestroy, self).__init__(name)
