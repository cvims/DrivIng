...
class PrivilegedRoutePlanner(object):
    ...
    def _smooth_transition(self, value, mode="cos"):
        """
        Transforms the linear between 0 to 1 into a curve one.

        Args:
            value (float): The input value between 0 and 1.
            mode (str): The mode of the transition. ["cos", "sigmoid", "linear"]
        ...
        if mode == "cos":
          ...
          p = 1.4
          numerator = value**p
          denominator = numerator + (1-value)**p
    ...
    def get_lane_offset_position(self, waypoint, offset):
        """
        Get the position of a waypoint offset by a certain distance along the lane.

        Args:
            waypoint (carla.Waypoint): The waypoint from which to calculate the offset position.
            offset (float): The distance to offset the position along the lane.

        Returns:
            np.ndarray: The location of the waypoint offset by the specified distance.
        """
        end_loc = waypoint
        
        if offset > 0:
            for _ in range(int(offset)):
                end_loc = end_loc.get_right_lane()
            lane_transition_factor = offset % 1
            if lane_transition_factor > 1e-3:
                loc = end_loc.get_right_lane()
                end_loc = lane_transition_factor * loc + (1 - lane_transition_factor) * end_loc
        elif offset < 0:
            for _ in range(int(-offset)):
                end_loc = end_loc.get_left_lane()
            lane_transition_factor = -offset % 1
            if lane_transition_factor > 1e-3:
                loc = end_loc.get_left_lane()
                end_loc = lane_transition_factor * loc + (1 - lane_transition_factor) * end_loc
            
        wp_loc = end_loc.transform.location
        wp_loc = np.array([wp_loc.x, wp_loc.y, wp_loc.z])
        return wp_loc
