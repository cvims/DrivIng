...
@TRANSFORMS.register_module()
class PhotoMetricDistortionMultiView(BaseTransform):
    """Apply the same photometric distortions to all camera views."""
    ...
    def transform(self, results):
        ...
        # sample global parameters ONCE
        b_delta = np.random.uniform(-self.brightness_delta, self.brightness_delta)
        ...
        for im in imgs:
            ...
            # brightness
            if abs(b_delta) > 1e-3:
                im = ImageEnhance.Brightness(im).enhance(1 + b_delta/255.0)
            ...
            if abs(hue) > 1e-3:
                ...
                hsv[..., 0] = (hsv[..., 0].astype(int) + int(hue)) % 256
                im = Image.fromarray(hsv, "HSV").convert("RGB")

            new_imgs.append(np.array(im).astype(np.float32))
