...
def load_gt(nusc: NuScenesDrivIng,
            eval_split: str,
            box_cls,
            verbose: bool = False,
            dataset_type: str = 'driving') -> EvalBoxes:
    """
    ...
    :param box_cls: Type of box to load, e.g. DetectionBox or TrackingBox.
    :param verbose: Whether to print messages to stdout.
    :param dataset_type: Type of dataset, e.g., 'carla' or "driving".
    :return: The GT boxes.
    """
    # Init.
    ...
    splits = create_splits_scenes(dataset_type, verbose=verbose)

    # Check compatibility of split with nusc_version.
    version = nusc.version
    if eval_split in {'train', 'val', 'train_detect', 'train_track'}:
        assert version.endswith('trainval'), \
            'Error: Requested split {} which is not compatible with NuScenesDrivIng version {}'.format(eval_split, version)
        ...
        raise ValueError('Error: Requested split {} which this function cannot map to the correct NuScenesDrivIng version.'
                         .format(eval_split))
