    ...
    def get_sample_data(self, sample_data_token: str,
        ...
        for box in boxes:
            if use_flat_vehicle_coordinates:
                ...
                if len(cs_record['rotation']) == 4:
                    box.rotate(Quaternion(cs_record['rotation']).inverse)
                elif len(cs_record['rotation']) == 3:
                    # left-hand transformation 
                    rot = cs_record['rotation']
                    rot = np.linalg.inv(np.array(rot))
                    rot_4x4 = np.eye(4)
                    rot_4x4[0:3, 0:3] = rot

                    box_transform = np.eye(4)
                    box_transform[0:3, 3] = box.center
                    box_transform[:3, :3] = box.orientation.rotation_matrix
                    box_transform = rot_4x4 @ box_transform
                    yaw_rad = np.arctan2(box_transform[1, 0], box_transform[0, 0])

                    box.center = box_transform[0:3, 3]
                    box.orientation = Quaternion(axis=[0, 0, 1], angle=yaw_rad)

                    box_vel = np.hstack([box.velocity, 1])
                    box.velocity = (rot_4x4 @ box_vel)[0:3]
