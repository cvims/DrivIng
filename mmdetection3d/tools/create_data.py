...
def nuscenes_driving_data_prep(root_path,
    ...
    """
    if subfolder:
        root_path = osp.join(root_path, subfolder)
        out_dir = osp.join(out_dir, subfolder)
    ...
    if version == 'v1.0-test':
        info_test_path = osp.join(out_dir, f'{info_prefix}_infos_test.pkl')
        update_pkl_infos('nuscenes-driving', out_dir=out_dir, pkl_path=info_test_path, subfolder=subfolder)
        return

    info_train_path = osp.join(out_dir, f'{info_prefix}_infos_train.pkl')
    info_val_path = osp.join(out_dir, f'{info_prefix}_infos_val.pkl')
    update_pkl_infos('nuscenes-driving', out_dir=out_dir, pkl_path=info_train_path, subfolder=subfolder)
    update_pkl_infos('nuscenes-driving', out_dir=out_dir, pkl_path=info_val_path, subfolder=subfolder)
    create_groundtruth_database(dataset_name, root_path, info_prefix,
                                f'{info_prefix}_infos_train.pkl')
...
parser.add_argument(
    '--subfolder',
    type=str,
    help='To use the correct recording. Only used for nuscenes-driving!'
)
