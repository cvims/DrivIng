...
def create_nuscenes_driving_infos(root_path,
    ...
    train_val_split = splits.create_splits_scenes()
    if version == 'v1.0-trainval':
        train_scenes = train_val_split["train"]
        val_scenes = train_val_split["val"]
    elif version == 'v1.0-test':
        train_scenes = train_val_split["test"]
        val_scenes = []
    else:
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmengine.track_iter_progress(nusc.sample):
        ...
        l2e_r_mat = np.array(l2e_r) if len(l2e_r) == 3 else Quaternion(l2e_r).rotation_matrix
        e2g_r_mat = np.array(e2g_r) if len(e2g_r) == 3 else Quaternion(e2g_r).rotation_matrix
...
def obtain_sensor2top(nusc,
    ...
    l2e_r_s_mat = np.array(l2e_r_s) if len(l2e_r_s) == 3 else Quaternion(l2e_r_s).rotation_matrix
    e2g_r_s_mat = np.array(e2g_r_s) if len(e2g_r_s) == 3 else Quaternion(e2g_r_s).rotation_matrix
