...
def convert_rotmat_to_transform(rotmat, translation):
    """Convert rotation matrix and translation to transformation matrix.

    Args:
        rotmat (np.ndarray): Rotation matrix with shape (3, 3).
        translation (np.ndarray): Translation with shape (3, ).
    Returns:
        np.ndarray: Transformation matrix with shape (4, 4).
    """
    transform = np.eye(4)
...
def update_nuscenes_driving_infos(pkl_path, out_dir, subfolder):
    camera_types = [
        'back_left_camera',
        'left_camera',
        'front_left_camera',
    ...
    nusc = NuScenesDrivIng(
        version=data_list['metadata']['version'],
        dataroot=f'./data/nuscenes-driving/{subfolder}' if subfolder else './data/nuscenes-driving',
        verbose=True)
    ...
    for i, ori_info_dict in enumerate(
        ...
        if len(ori_info_dict['ego2global_rotation']) == 3:
            temp_data_info['ego2global'] = convert_rotmat_to_transform(
                ori_info_dict['ego2global_rotation'],
                ori_info_dict['ego2global_translation'])
        else:
            temp_data_info['ego2global'] = convert_quaternion_to_matrix(
        ...
        if len(ori_info_dict['lidar2ego_rotation']) == 3:
            temp_data_info['lidar_points'][
                'lidar2ego'] = convert_rotmat_to_transform(
                    ori_info_dict['lidar2ego_rotation'],
                    ori_info_dict['lidar2ego_translation'])
        else:
            temp_data_info['lidar_points'][
        ...
        for ori_sweep in ori_info_dict['sweeps']:
            temp_lidar_sweep = get_single_lidar_sweep()
            if len(ori_sweep['sensor2ego_rotation']) == 3:
                temp_lidar_sweep['lidar_points'][
                    'lidar2ego'] = convert_rotmat_to_transform(
                        ori_sweep['sensor2ego_rotation'],
                        ori_sweep['sensor2ego_translation'])
            ...
            if len(ori_sweep['ego2global_rotation']) == 3:
                temp_lidar_sweep['ego2global'] = convert_rotmat_to_transform(
                    ori_sweep['ego2global_rotation'],
                    ori_sweep['ego2global_translation'])
            else:
                temp_lidar_sweep['ego2global'] = convert_quaternion_to_matrix(
        ...
        for cam in ori_info_dict['cams']:
            ...
            empty_img_info[
                'timestamp'] = ori_info_dict['cams'][cam]['timestamp'] / 1e6
            if len(ori_info_dict['cams'][cam]['sensor2ego_rotation']) == 3:
                empty_img_info['cam2ego'] = convert_rotmat_to_transform(
                    ori_info_dict['cams'][cam]['sensor2ego_rotation'],
                    ori_info_dict['cams'][cam]['sensor2ego_translation'])
            else:
                empty_img_info['cam2ego'] = convert_quaternion_to_matrix(
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='',
        required=False,
        help='extra for dataset, e.g. for nuscenes-driving "day".'
    )
    args = parser.parse_args()
    return args


def update_pkl_infos(dataset, out_dir, pkl_path, subfolder=None):
    if dataset.lower() == 'kitti':
        ...
        update_nuscenes_driving_infos(pkl_path=pkl_path, out_dir=out_dir, subfolder=subfolder)
...
if __name__ == '__main__':
    ...
    update_pkl_infos(
        dataset=args.dataset, out_dir=args.out_dir, pkl_path=args.pkl_path, subfolder=args.subfolder)
